...
double wartosc_sin(double,double,int);
double *tab;
double suma=0; 
double a=0;
double b=180;
...
void *suma_w( void *arg_wsk){
  ...
  moja_suma += wartosc_sin((b-a)/LICZBA_W*moj_id,(b-a)/LICZBA_W*(moj_id+1),N/LICZBA_W); 
...
double wartosc_sin(double c, double d,int e)
{
double dx=(d-c)/e;
...
for(int i=0;i<N-1;i++)
{
calka+=(dx*0.5*(sin(X1*3.14/180)+sin((X1+dx)*3.14/180)));
X1+=dx;
